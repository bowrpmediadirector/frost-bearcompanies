...
const validPlatforms = new Set(['youtube', 'twitch']);
...
app.post('/api/notifications/:guildId', dashboardAuth, async (req, res) => {
  ...
  if (!validPlatforms.has(String(platform).toLowerCase())) {
    return res.status(400).json({ ok: false, message: 'platform must be youtube or twitch.' });
  }
